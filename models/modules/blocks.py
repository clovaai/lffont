...
#      """ pre-active single res block """
#      def __init__(self, C_in, C_out, kernel_size=3, stride=1, padding=1, norm='none',
#                   activ='relu', bias=True, upsample=False, w_norm='none', pad_type='zero'):
#          # 1x1 conv assertion
#          if kernel_size == 1:
#              assert padding == 0
#          super().__init__()
...
#          self.conv = ConvBlock(C_in, C_out, kernel_size, stride, padding, norm, activ, bias,
#                                upsample, w_norm, pad_type)

#          if C_in != C_out or upsample:
...
class Upsample1x1(nn.Module):
    ...
    def __init__(self, C_in, C_out, norm='none', activ='relu', w_norm='none'):
        ...
        self.C_out = C_out
        self.proj = ConvBlock(
            C_in, C_out*4, 1, 1, 0, size=1, norm=norm, activ=activ, w_norm=w_norm
        )
...
class HourGlass(nn.Module):
    ...
    def __init__(self, C_in, C_max, size, n_downs, n_mids=1, block_type='conv', down_stride=True,
                 norm='none', activ='relu', w_norm='none', pad_type='zero', use_up1x1=False,
                 norm1x1=None):
        """
        Args:
        ...
        self.mids = nn.Sequential(
            *[
                ConvBlk(c_in, c_out, kernel_size=1, padding=0, size=size, norm=mid_norm)
